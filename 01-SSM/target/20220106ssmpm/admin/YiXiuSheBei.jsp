    ...
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            <li class="page-item ${!info.hasPreviousPage ? 'disabled' : ''}">
                <a class="page-link" href="<c:if test="${info.hasPreviousPage}"><c:url value='/admin/chaKanYiXiuXinXi.action'/>?page=${info.prePage}</c:if>" aria-label="Previous">
            ...
            <c:forEach begin="${startPage}" end="${endPage}" var="pageNum">
                <li class="page-item ${info.pageNum == pageNum ? 'active' : ''}">
                    <a class="page-link" href="<c:url value='/admin/chaKanYiXiuXinXi.action'/>?page=${pageNum}">${pageNum}</a>
                </li>
            </c:forEach>
            <li class="page-item ${!info.hasNextPage ? 'disabled' : ''}">
                <a class="page-link" href="<c:if test="${info.hasNextPage}"><c:url value='/admin/chaKanYiXiuXinXi.action'/>?page=${info.nextPage}</c:if>" aria-label="Next">
...
<script>
    ...
    function anzhuang(id) {
        window.location.href = "${pageContext.servletContext.contextPath}/admin/anzhuangPost.action?id=" + id;
    }
</script>
</body>
</html>