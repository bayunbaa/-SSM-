...
<center>
    <div style="float: right;margin: 5px;">
        <a class="btn btn-default" href="<c:url value="/admin/addTeacher.jsp"/>" type="button" value="添加人员">添加人员</a>
    </div>
    <div id="condition" style="text-align: center">
        <form id="myform" action="${pageContext.request.contextPath}/admin/findTeacher.action" method="get">
    ...
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            <li class="page-item ${!info.hasPreviousPage ? 'disabled' : ''}">
                <a class="page-link" href="<c:if test="${info.hasPreviousPage}"><c:url value='/admin/findTeacher.action'/>?page=${info.prePage}</c:if>" aria-label="Previous">
            ...
            <c:forEach begin="${startPage}" end="${endPage}" var="pageNum">
                <li class="page-item ${info.pageNum == pageNum ? 'active' : ''}">
                    <a class="page-link" href="<c:url value='/admin/findTeacher.action'/>?page=${pageNum}">${pageNum}</a>
                </li>
            </c:forEach>
            <li class="page-item ${!info.hasNextPage ? 'disabled' : ''}">
                <a class="page-link" href="<c:if test="${info.hasNextPage}"><c:url value='/admin/findTeacher.action'/>?page=${info.nextPage}</c:if>" aria-label="Next">
