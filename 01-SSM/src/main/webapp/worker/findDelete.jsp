...
<head>
    ...
    <script src="../js/jquery.slim.min.js" integrity="sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUUew+OrCXaRkfj" crossorigin="anonymous"></script>
...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-10 table-container">
            <h1 class="text-center mb-4">设备报废</h1>
            <div id="condition" style="text-align: center">
                <form id="myform" action="${pageContext.request.contextPath}/teacher/findDelete.action" method="get">
            ...
            <table class="table table-striped table-hover">
                ...
                <c:forEach items="${info.list}" var="c" varStatus="a">
                    <tr>
                        ...
                        <td>${c.createtime}</td>
                        <td>
                            <a href="${pageContext.servletContext.contextPath}/worker/delete.action?id=${c.id}">报废</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
    ...
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            <li class="page-item ${!info.hasPreviousPage ? 'disabled' : ''}">
                <a class="page-link" href="<c:if test="${info.hasPreviousPage}"><c:url value='/worker/deleteFacility.action'/>?page=${info.prePage}</c:if>" aria-label="Previous">
            ...
            <c:forEach begin="${startPage}" end="${endPage}" var="pageNum">
                <li class="page-item ${info.pageNum == pageNum ? 'active' : ''}">
                    <a class="page-link" href="<c:url value='/worker/deleteFacility.action'/>?page=${pageNum}">${pageNum}</a>
                </li>
            </c:forEach>
            <li class="page-item ${!info.hasNextPage ? 'disabled' : ''}">
                <a class="page-link" href="<c:if test="${info.hasNextPage}"><c:url value='/worker/deleteFacility.action'/>?page=${info.nextPage}</c:if>" aria-label="Next">
